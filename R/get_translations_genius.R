...
#'
#' @examples
get_translations_genius <- function(
  input,
  g_token,
  language = 'en',
  checkpointstep = 100
  ...
  if (last_index > 0) {
    input_distinct <- tail(input_distinct, -last_index)
  }
  purrr::pmap_df(
    list(input_distinct$track.g.id, input_distinct$track.g.lyrics),
    get_translation_for_single_track %>%
      save_checkpoint_and_count(
        checkpoint_name,
        last_index,
        saved_data,
        savingstep = checkpointstep,
...
get_translation_for_single_track <- function(
  track.g.id,
  track.g.lyrics,
  g_token,
  language
  ...
  res %>%
    dplyr::rename(!!paste0('track.g.lyrics_', language) := 'lyrics_translation')
}
...
get_language_details <- function(track.g.id, g_token) {
  ...
  tibble::tibble(
    track.g.id,
    track.g.language = ifelse(
      is.null(res$response$song$language),
      NA,
      res$response$song$language
    ),
    track.g.translations = ifelse(
      is.null(list(res$response$song$translation_songs)),
      NA,
...
copy_lyrics_already_correct_language <- function(
  res,
  track.g.lyrics,
  language
) {
  dplyr::mutate(
    res,
    lyrics_translation = ifelse(
      track.g.language == language,
      list(track.g.lyrics),
      NA
    )
  )
...
get_available_languages <- function(input) {
  available <- input %>%
    dplyr::mutate(
      translations = purrr::map(
        track.g.translations,
        ~ {
          res <- tibble::tibble(
